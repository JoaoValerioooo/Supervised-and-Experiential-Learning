...
class PREPROCESS:
    ...
    def discretize(self, df):
        """
        Discretizes the numerical columns of a dataframe using K-Means discretization.

        Parameters:
        - df: pandas dataframe
            Input dataframe with numerical columns to be discretized

        Returns:
         - df: pandas dataframe
             Dataframe with numerical columns discretized using K-Means discretization
        """
        numerical = []
        for ind, col in enumerate(df.columns.values):
            # Check if column is numerical and add index of numerical column to list
            if df[col].dtype != object: numerical.append(ind)
        # If no numerical columns found, return original dataframe
        if len(numerical) == 0: return df
        # Create KBinsDiscretizer object with 10 bins, 'ordinal' encoding, and 'quantile' strategy
        kbins = KBinsDiscretizer(n_bins=10, encode='ordinal', strategy='quantile')
        # Discretize numerical columns in dataframe using KBinsDiscretizer
        df.iloc[:, numerical] = kbins.fit_transform(df.iloc[:, numerical])
        return df
    ...
    def preprocessDataset(self, filename):
        ...
        # Discretize the numerical columns of the dataset
